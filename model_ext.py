...
class ValidDepthCompletionNet(nn.Module):
    ...
    def create_from_zoo(self, layers=18,pretrained=True):
        ...
        assert ( layers in [18, 34, 50, 101, 152]), 'Only layers 18, 34, 50, 101, and 152 are defined, but got {}'.format(layers)
        super(ValidDepthCompletionNet, self).__init__()
        ...
        self.convt1 = convt_bn_relu(in_channels=128, out_channels=64,
            kernel_size=kernel_size, stride=1, padding=1)
        self.convtf_d = conv_bn_relu(in_channels=128, out_channels=1, kernel_size=1, stride=1, bn=False, relu=False)
        self.convtf_v = conv_validation(in_channels=128)
    ...
    def load_from_depth_completion_net(self,pretrained_model):
        ...
        self.convt2 = pretrained_model.convt2
        self.convt1 = pretrained_model.convt1
        self.convtf_d = pretrained_model.convtf
        self.convtf_v = conv_validation(in_channels=128)
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            return F.relu(y_d), torch.sigmoid(y_v)
        return y
